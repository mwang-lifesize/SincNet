  ...
  if epoch%N_eval_epoch==0:
   ...
   with torch.no_grad():  
    ...
    for i in range(snt_te):
     ...
     #signal=signal.astype(float)/32768
     # use random file as snt_te could get bigger, we want to speed up
     ...
     # a fix so that we can cacluate prop correctly
     total_fr = N_fr
     if remainder > 0:
      total_fr = N_fr + 1
   ...
   torch.save(checkpoint,output_folder+'/' + str(epoch) + '_model_raw.pkl')
